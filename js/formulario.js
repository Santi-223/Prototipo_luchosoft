...
const validateEmailFormat = e =>{
 ...
 if(fieldValue.trim().length >5 && !regex.test(fieldValue)){
   field.classList.add("invalid");
   field.nextElementSibling.classList.add ("error");
   field.nextElementSibling.innerText ="Ingrese un correo electrónico válido";
...
const validateNombFormat = e =>{
   ...
   if(fieldValue.trim().length >1 && !regex.test(fieldValue)){
     field.classList.add("invalid");
     field.nextElementSibling.classList.add ("error");
     field.nextElementSibling.innerText ="Ingrese un nombre válido.";
...
const validateDirecFormat = e =>{
   ...
   if(fieldValue.trim().length >4 && !regex.test(fieldValue)){
     field.classList.add("invalid");
     field.nextElementSibling.classList.add ("error");
     field.nextElementSibling.innerText ="Ingrese una dirección válido.";
...
const validateContraseñaFormat = e =>{
   const field = e.target;
   const fieldValue = e.target.value;
   const regex= new RegExp (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/)
   if(fieldValue.trim().length >8 && !regex.test(fieldValue)){
     field.classList.add("invalid");
     field.nextElementSibling.classList.add ("error");
     field.nextElementSibling.innerText ="Ingrese máximo 8 dígitos que contegan números, mayusculas y minusculas";
  ...
  password.addEventListener("input",validateContraseñaFormat);